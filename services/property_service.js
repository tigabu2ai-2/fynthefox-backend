...
class PropertyService {
  ...
  async fetch_by_id(id) {
    const property = await Property.findByPk(id, {
      ...
      include: {
        model: Address,
        attributes: ["id", "country", "state", "city", "street", "zip_code"],
      },
    });

    if (!property) throw new CustomException("Property not found!");
  ...
  async update(id, data) {
    ...
    if (data.address) {
      property.Address.country =
        data.address.country ?? property.Address.country;
      property.Address.state = data.address.state ?? property.Address.state;
      property.Address.city = data.address.city ?? property.Address.city;
      property.Address.street = data.address.street ?? property.Address.street;
      property.Address.zip_code =
        data.address.zip_code ?? property.Address.zip_code;

      await property.Address.save();
    }
  ...
  async assign_vendor(property_id, vendor_id, requester_id) {
    ...
    if (!vendor_property)
      throw new CustomException("Failed to assign vendor! Please try again.");
  ...
  async retract_vendor(property_id, vendor_id, requester_id) {
    ...
    if (!vendor_property)
      throw new CustomException(
        "Vendor was not assigned to the provided property!",
        400
      );
