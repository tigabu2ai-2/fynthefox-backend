...
class UserService {
  async register(data, role_name, created_by) {
    ...
    if (user_exist) {
      throw new CustomException(
        `A user already exist with the email ${data.email}`,
        400
      );
    }
        ...
        PropertyManagers: [
          {
            ...
            password_hash: data.password_hash,
            role_id: (
              await Role.findOne({ where: { name: "property-owner" } })
            ).id,
            created_by: created_by,
          },
        ],
    ...
    const tenant_info = await TenantInfo.create(
      {
        ...
        Tenant: {
          ...
          status: "active",
          role_id: (
            await Role.findOne({ where: { name: "property-user" } })
          ).id,
  ...
  async resident_exist(user_id) {
    return !!(await User.findByPk(user_id, {
      include: { model: Role, where: { name: "property-user" } },
    }));
  }
  ...
  async update_property_user(user_id, data) {
    ...
    user.TenantInfo.property_id =
      data.property_id ?? user.TenantInfo.property_id;
    user.TenantInfo.floor_number =
      data.floor_number ?? user.TenantInfo.floor_number;
    user.TenantInfo.apartment_number =
      data.apartment_number ?? user.TenantInfo.apartment_number;
  ...
  async create_vendor(data, created_by) {
    ...
    if (!manager.company_info_id)
      throw new CustomException(
        "You are not authorized to create vendor! Contact admin.",
        403
      );
  ...
  async update_vendor(user_id, data, manager_id) {
    ...
    vendor.VendorInfo.availability =
      data.availability ?? vendor.VendorInfo.availability;
    (vendor.VendorInfo.service_area =
      data.service_area ?? vendor.VendorInfo.service_area),
      (vendor.VendorInfo.preferred_contact_method =
  ...
  async agent_fetch_all_vendors(agent_id, query) {
    ...
    const sanitized_vendors = vendors.map((vendor) => {
      return {
        ...
        properties: vendor.VendorInfo.Properties.map((property) => {
          return property.id;
        }),
      };
    });
  ...
  async vendor_created_or_modified(vendor_id, company_info_id) {
    try {
      ...
      switch (action) {
        case "create":
          await AgentJunkHandler.create_user(
            sanitized_vendor,
            agent.junk_schema_name
          );
          break;
        case "update":
          await AgentJunkHandler.update_user(
            sanitized_vendor,
            agent.junk_schema_name
          );
          break;
